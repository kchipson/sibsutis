...
def cubic_spline(x_arr: Tuple[D], y_arr: Tuple[D], M: List[D], h: Tuple[D], x: D):
    ...
    m1 = M[i - 1] * ((x_arr[i] - x)**3 / (D("6") * h[i - 1]))
...
def main():
    ...
    x_g: Tuple[D] = tuple(sorted(set(x_init + tuple(D(i) for i in np.arange(x_init[0], x_init[-1] + step_g, step_g)))))
    ...
    if fun:
        ...
        canvas.plot(x_init, y_init,    linestyle="-", color="r", label=f"f(x) = {fun}")
    

    canvas.plot(x_init, y_init,    linestyle="-", color="g", label="f(x) = {init}")
