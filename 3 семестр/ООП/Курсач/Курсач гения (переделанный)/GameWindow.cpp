...
GameWindow::GameWindow()
{
  ...
  moveText.setFont(Assets::Instance().fontIron) ;
  moveText.setCharacterSize(50) ;
  moveText.setOutlineThickness(5.f) ;
...
void GameWindow::currentMoveText()
{
  if (blinkTimer.getElapsedTime().asSeconds() >= BLINK_PERIOD)
    blinkTimer.restart() ;

  if (gameState == playerShot)
  {
    moveText.setString(L"Ход игрока");
    moveText.setPosition(490, 520) ;
    moveText.setOutlineColor(sf::Color(0, 255, 0, (255 * blinkTimer.getElapsedTime().asSeconds() / BLINK_PERIOD))) ;
  }

  if (gameState == computerShot)
  {
    moveText.setString(L"Ход компьютера");
    moveText.setPosition(425, 520) ;
    moveText.setOutlineColor(sf::Color(255, 0, 0, (255 * blinkTimer.getElapsedTime().asSeconds() / BLINK_PERIOD))) ;
  }
  if (gameState == playerEndGame)
  {
    moveText.setString(L"победа игрока");
    moveText.setFont(Assets::Instance().fontMagnum);
    moveText.setCharacterSize(96);
    moveText.setPosition(290, 200) ;
    moveText.setOutlineThickness(10.f);
    moveText.setOutlineColor(sf::Color(80, 35, 224, (255 * blinkTimer.getElapsedTime().asSeconds() / BLINK_PERIOD * 2))) ;
  }

  if (gameState == computerEndGame)
  {
    moveText.setString(L"победа компьютера");
    moveText.setFont(Assets::Instance().fontMagnum);
    moveText.setCharacterSize(96);
    moveText.setPosition(180, 200) ;
    moveText.setOutlineThickness(10.f);
    moveText.setOutlineColor(sf::Color(239, 0, 150, (255 * blinkTimer.getElapsedTime().asSeconds() / BLINK_PERIOD * 2))) ;
  }
}
...
void GameWindow::setShipOnField(sf::Vector2i mousePosition)
{
  for (uint8_t x = 0 ; x < playerBoard->getSize() ; x++)
  {
    for (uint8_t y = 0 ; y < playerBoard->getSize() ; y++)
...
void GameWindow::placementShipsByComputer()
{
  for (uint8_t i = 0 ; i < NUM_SHIPS ; i++)
  {
    ...
    do
    {
      x = rand() % computerBoard->getSize() ;
      y = rand() % computerBoard->getSize() ;
      if (rand() % 2)
  ...
  for (uint8_t x = 0 ; x < computerBoard->getSize() ; x++)
  {
    for (uint8_t y = 0 ; y < computerBoard->getSize() ; y++)
    {
      if (sf::IntRect(computerBoard->getGrid()[x][y].getSprite().getGlobalBounds()).contains(mousePosition))
      {
        ...
        {
          for (uint8_t ship = 0 ; ship < NUM_SHIPS ; ship++)
...
int8_t GameWindow::computerMakeShot(sf::Clock &thinkingTimer)
{
  ...
  do
  {
    x = rand() % playerBoard->getSize() ;
    y = rand() % playerBoard->getSize() ;
...
{
  bool isEndgamePlayer = true;
  bool isEndgameComputer = true;
  for (uint8_t x = 0; x < NUM_SHIPS; x++)
  {
    if (!playerBoard->getShips()[x].isSunken())
      isEndgamePlayer = false;
    if (!computerBoard->getShips()[x].isSunken())
      isEndgameComputer = false;
  }

  return !(!isEndgamePlayer and !isEndgameComputer);
}
...
int GameWindow::draw(sf::RenderWindow &window)
{
  ...
  sf::Clock thinkTime ;
  sf::RectangleShape zRectangle(sf::Vector2f(window.getSize().x, window.getSize().y));
  zRectangle.setPosition(sf::Vector2f(0, 0));
  zRectangle.setFillColor(sf::Color(23, 17, 20, 200));
  ...
  while (true)
  {
    ...
    currentMoveText() ; // Задание свойств
    ...
    if (gameState == playerEndGame)
    {
      window.draw(zRectangle) ; // Затемняющая маска
      window.draw( moveText);
