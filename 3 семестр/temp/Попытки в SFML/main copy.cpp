...
void Draw(sf::RenderWindow &window, TREE *tree, float x, float y) {
  ...
  if (tree != NULL) {
    ...
    if (tree->L != NULL) {
      sf::Vertex line[] = {sf::Vertex(sf::Vector2f(x - w / 4, y + h / 2)),
                           sf::Vertex(sf::Vector2f(
                               x - w / 2 * pow(2, depth) - w / 2, y + h * 3))};
      line[0].color = sf::Color::Red;
      line[1].color = sf::Color::Red;
      window.draw(line, 2, sf::Lines);
      Draw(window, tree->L, x - w / 2 * pow(2, depth) - w / 2,
           y + h * 3 + h / 2);
    }
    if (tree->R != NULL) {
      sf::Vertex line[] = {sf::Vertex(sf::Vector2f(x + w / 4, y + h / 2)),
      ...
      line[1].color = sf::Color::Red;
      window.draw(line, 2, sf::Lines);
      Draw(window, tree->R, x + w / 2 * pow(2, depth) + w / 2,
           y + h * 3 + h / 2);
    }
  }
}
...
int window(TREE *tree) {
  ...
  while (window.isOpen()) {
    ...
    Draw(window, tree, window.getSize().x / 2, window.getSize().y / 20);
    window.display();
  }
}
